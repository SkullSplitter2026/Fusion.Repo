...
k_version =int (xbmc .getInfoLabel ('System.BuildVersion').split ('.')[0 ])
def _make_heading (heading =None ):
    return heading if heading else ADDON_NAME

def refresh ():
    set_kodi_string ('evil_refresh','1')
...
def exception (heading =None ):
    ...
    include =[ADDON_ID ,os .path .join (COMMON_ADDON_ID ,'resources','modules','evil'),os .path .join (COMMON_ADDON_ID ,'resources','lib')]
    fline =True
...
class Item (object ):
    ...
    def get_li (self ):
        ...
        if self .info :
            ...
            if k_version >=20 :
                li .getVideoInfoTag (info )
            else :li .setInfo ('video',info )
        ...
        if self .path and (self .path .lower ().startswith ('http://')or self .path .lower ().startswith ('https://')):
            ...
            set_kodi_string ('_evil_quality',json .dumps (proxy_data ))

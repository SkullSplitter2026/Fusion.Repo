...
import xbmc ,xbmcplugin ,xbmcvfs
...
def _add_bookmark (path ,label =None ,thumb =None ,folder =1 ,playable =0 ,fanart =None ,poster =None ,series_title =None ,**kwargs ):
    bookmarks .add (path ,label ,thumb ,int (folder ),int (playable ),fanart =fanart ,poster =poster ,series_title =series_title )
...
class Item (gui .Item ):
    ...
    def get_li (self ,*args ,**kwargs ):
        ...
        if settings .getBool ('bookmarks')and self .bookmark :
            ...
            'path':self .path ,
            'label':self .label ,
            'thumb':self .art .get ('thumb'),
            'folder':int (self .is_folder ),
            'playable':int (self .playable ),
            }
            if self .art .get ('fanart'):bm_kwargs ['fanart']=self .art ['fanart']
            if self .art .get ('poster'):bm_kwargs ['poster']=self .art ['poster']
            series_title =self .info .get ('tvshowtitle','')or self .info .get ('TVShowTitle','')
            if series_title :bm_kwargs ['series_title']=series_title
            url =url_for (ROUTE_ADD_BOOKMARK ,**bm_kwargs )
    ...
    def play (self ,**kwargs ):
        ...
        set_kodi_string ('_evil_play_data',json .dumps (play_data ))

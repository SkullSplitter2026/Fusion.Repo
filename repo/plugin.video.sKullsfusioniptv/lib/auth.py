...
class Auth:
    ...
    def get_token(self, refresh_token):
        ...
        response = requests.get(url=self.__url,
                                headers={'Cookie': self.__mac_cookie, 'X-User-Agent': 'Model: MAG250; Link: WiFi', 'Referer': self.__referrer,
        ...
        if response.status_code != 200 or response.text.find('Authorization failed') != -1:
            ...
            # Portal als "Fehler" markieren (für rote Einfärbung im Menü)
            try:
                addon = xbmcaddon.Addon()
                suffix = '' if int(G.active_portal) == 1 else f'_{int(G.active_portal)}'
                addon.setSetting(f'portal_error{suffix}', 'true')
            except Exception as e:
                Logger.debug(f"Could not persist portal_error flag: {str(e)}")

            # Kein Dialog mehr -> nicht blockieren, Fehler nur loggen und Exception werfen
            raise Exception

        self.__token.value = response.json()['js']['token']

        # Token ok -> Portal-Fehlerstatus zurücksetzen
        try:
            addon = xbmcaddon.Addon()
            suffix = '' if int(G.active_portal) == 1 else f'_{int(G.active_portal)}'
            addon.setSetting(f'portal_error{suffix}', 'false')
        except Exception as e:
            Logger.debug(f"Could not reset portal_error flag: {str(e)}")
    ...
    def __refresh_token(self):
        ...
        requests.get(url=self.__url,
                     headers={'Cookie': self.__mac_cookie, 'SN': G.portal_config.serial_number, 'Authorization': 'Bearer ' + self.__token.value,
                              'X-User-Agent': 'Model: MAG250; Link: WiFi', 'Referer': self.__referrer,
                              'User-Agent': 'Mozilla/5.0 (QtEmbedded; U; Linux; C) AppleWebKit/533.3 (KHTML, like Gecko) MAG200 stbapp ver: 2 rev: 250 Safari/533.3'},
                     params={
                         'type': 'stb',
                         'action': 'get_profile',

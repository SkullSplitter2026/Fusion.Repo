...
def UpdateResolve(username, resolve_dir, resolve_id, branch, token):
    ...
    try:
        ...
        commitXML = _getXmlString(REMOTE_PLUGIN_COMMITS, auth, sessionUA)  # Commit Update
        if commitXML:
...
def doUpdate(LocalDir, REMOTE_PATH, Title, localFileName, auth, sessionUA):
    try:
        response = requests.get(REMOTE_PATH, auth=auth, headers={'User-Agent': sessionUA}, timeout=8)  # verify=False,
...
def _getXmlString(xml_url, auth, sessionUA):
    try:
        xmlString = requests.get(xml_url, auth=auth, headers={'User-Agent': sessionUA}, timeout=4).content  # verify=False,

...
#
# Phase 0 (xStream only): TMDB title+year -> tmdb_id
...
from resources.lib.handler.requestHandler import cRequestHandler  # RandomUA() — Memory #15
...
_PROP_PREFIX = '%s.trailer' % _ADDON_NAME     # window property prefix: xstream.trailer or xship.trailer
...
def _extractSeasonFromTitle(title):
    ...
    Matched (in priority order, case-insensitive, DE + EN):
      - Zahl vorne:   "2. Staffel", "4. Season"
      - Zahl hinten:  "Staffel 3", "Season 4"

    Returns the season number as int, or None if no match.
    """
    # "Zahl vorne" muss zuerst geprueft werden, sonst klaut das zweite
    # Pattern evtl. eine andere Ziffer im Titel.
    m = re.search(r'(\d+)\.\s*(?:Staffel|Season)', title, re.I)
    if m:
        return int(m.group(1))
    # Trenner zwischen Wort und Zahl: Whitespace und/oder - bzw. : (z.B.
    # "Staffel 2", "Staffel-2", "Staffel - 2", "Staffel:2").
    m = re.search(r'(?:Staffel|Season)[\s:-]+(\d+)', title, re.I)
    if m:
        return int(m.group(1))
    return None
...
def _yt_season_suffix(lang, season):
    """Localized season suffix for the YouTube search query.
    DE -> 'Staffel N' (German uploads label seasons 'Staffel'); else 'Season N'."""
    word = 'Staffel' if (lang or '').lower().startswith('de') else 'Season'
    return '%s %s' % (word, season)


def _filterYouTubeBySeason(hits, season):
    """Drop YouTube hits whose title names a DIFFERENT season.
    Keeps exact-season matches and ambiguous titles (no season number in the name,
    e.g. 'Stranger Things 4 | Trailer'), so only positive mismatches are rejected."""
    if season is None:
        return hits
    out = []
    for h in hits:
        s = _extractSeasonFromTitle(h.get('name', ''))
...
# language/season field, so both get parsed from the video name downstream.
...
def _searchIMDBVideoStrip(imdb_id, season=None, lang='de'):
    """IMDB videoStrip lookup for a season-specific (or any) trailer in a given language.
    IMDB has no per-video language/season field, so both are parsed from the video name
    (e.g. 'River: Season 1 (German Trailer)' / 'Season 3 Official Trailer'): season via
    _extractSeasonFromTitle, language via name markers. lang='de' -> name MUST carry a
    ...
    runs at most once per language block per request). Shares _imdb_dead with _searchIMDB."""
    import json
    global _imdb_dead
    if not imdb_id or _imdb_dead:
        return ('', '', '')
    _log('IMDB videoStrip: %s season=%s lang=%s' % (imdb_id, season, lang))
    from urllib.request import Request, urlopen
    from urllib.error import HTTPError
    try:
    ...
    for e in edges:
        ...
        if lang != 'de' and is_de:
            continue
        # Season gate: name must name the exact season (IMDB labels season trailers).
        if season is not None and _extractSeasonFromTitle(name) != season:
            continue
        url, quality = _pickIMDBPlayback(node.get('playbackURLs', []))
        if url:
            _log('IMDB videoStrip match: %r q=%s' % (name, quality))
            return (url, quality, name)
    _log('IMDB videoStrip: no %s%s match for %s' % (
        lang, (' S%s' % season) if season else '', imdb_id))
    return ('', '', '')
...
def _runTrailerSearch(tmdb_id, mediatype, title, en_title, year, poster,
    ...
    for lang in blocks:
        ...
        # IMDB German trailer (DE block): videoStrip, name-filtered German + season.
        # No YT player/key needed (direct MP4/HLS). primaryVideos is EN/non-season,
        # so the German (and season-specific) case goes through videoStrip.
        if lang == 'de' and imdb_id and not _imdb_dead:
            step += 1
            _log('--- [DE] IMDB videoStrip ---')
            imdb_url, imdb_quality, imdb_name = _searchIMDBVideoStrip(imdb_id, season=season, lang='de')
            _log('[DE] IMDB-strip: url=%s quality=%s name=%r' % (
                imdb_url[:80] if imdb_url else '', imdb_quality, imdb_name))
            if imdb_url:
                _playDirect(imdb_url, step, 'IMDB', 'Trailer', 'DE', poster, imdb_name or title, vtype_prefix=vtype_prefix)
                return {'found_lang': 'DE', 'source': 'IMDB'}

        # IMDB direct MP4 (EN block): no player/key needed, ID-based.
        # Season -> videoStrip (primaryVideos is not season-specific); else primaryVideos.
        if lang == 'en' and imdb_id and not _imdb_dead:
            step += 1
            if season:
                _log('--- [EN] IMDB videoStrip ---')
                imdb_url, imdb_quality, imdb_name = _searchIMDBVideoStrip(imdb_id, season=season, lang='en')
                _log('[EN] IMDB-strip: url=%s quality=%s name=%r' % (
                    imdb_url[:80] if imdb_url else '', imdb_quality, imdb_name))
                play_title = imdb_name or en_title or title
            else:
                _log('--- [EN] IMDB ---')
            ...
            if imdb_url:
                _playDirect(imdb_url, step, 'IMDB', 'Trailer', '', poster, play_title, vtype_prefix=vtype_prefix)
    ...
    if has_yt_player and has_own_key:
        ...
        for yt_lang in languages:
            ...
            if season:
                yt_raw = _searchYouTube(yt_title, '', lang=yt_lang,
                                        search_suffix=_yt_season_suffix(yt_lang, season))
                # Drop hits whose title names a different season (keep match + ambiguous).
                _before = len(yt_raw)
                yt_raw = _filterYouTubeBySeason(yt_raw, season)
                _log('YouTube-%s: season-filter %d -> %d' % (yt_upper, _before, len(yt_raw)))
            else:
...
def playTrailer(tmdb_id, mediatype='movie', title='', year='', poster='', pref_lang='de', season=None):
    ...
    if _ADDON_NAME == 'xstream' and not tmdb_id:
        ...
        if not tmdb_id:
            ...
            is_de = (_ADDON_NAME == 'xship') or (xbmc.getLanguage(xbmc.ISO_639_1) == 'de')
            xbmcgui.Dialog().notification(
    ...
    if season:
        try:
            ...
            # imdb_id kept: IMDB has season-specific trailers in its videoStrip,
            # name-tagged (e.g. "Season 1 (German Trailer)"). primaryVideos stays
            # gated off for seasons in _runTrailerSearch; videoStrip handles them.
            _log('Season %s: tmdb_videos=%d results' % (
    ...
    is_de = (_ADDON_NAME == 'xship') or (xbmc.getLanguage(xbmc.ISO_639_1) == 'de')
    _season_prefix = 'Staffel-' if is_de else 'Season-'
    result = _runTrailerSearch(
        ...
        season=season,
        vtype_prefix=_season_prefix if season else '',
    )

    # Kein Serien-Trailer-Fallback bei Staffeln: ein Staffel-Klick liefert den Staffel-Trailer
    # oder nichts. Der Serien-Trailer ist ueber den Serien-Klick erreichbar. Verhindert, dass
    # jede trailerlose Staffel denselben generischen Serien-Trailer bekommt (Futurama-Fall).
    if result is None and season:
        _log('Season %s: kein Staffel-Trailer gefunden — kein Serien-Fallback (bewusst)' % season)

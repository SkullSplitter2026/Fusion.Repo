...
class cTMDB:
    ...
    def search_movie_name(self, name, year='', page=1, advanced='false'):
        name = re.sub(' +', ' ', name)
        # Aki: Sprach-/Versions-Suffix "... Ger Dub"/"... Ger Sub" am Ende entfernen
        # (z.B. animetoast-Filme). Originalcase hier -> case-insensitive.
        name = re.sub(r'\s+ger[\s-]+(?:dub|sub)\b\s*$', '', name, flags=re.I)
        if year:
    ...
    def search_tvshow_name(self, name, year='', page=1, genre='', advanced='false'):
        ...
        if '- staffel' in name:
            ...
            # Format B (z.B. Megakino): "show - 1 staffel" / "show - 12 staffel"
            name = re.sub(r'\s*-\s*\d+\s+staffel\s*$', '', name)
        elif 'staffel' in name:
            name = re.sub(r'\s\wtaffel[^>]([1-9\-]+)', '', name)
        # Aki: Sprach-/Versions-Suffix "... ger dub"/"... ger sub" am Ende entfernen
        # (z.B. animetoast; name ist hier bereits lowercased). Andere Sites mit dem
        # gleichen Suffix profitieren automatisch, ohne Site-Anpassung.
        name = re.sub(r'\s+ger[\s-]+(?:dub|sub)\b\s*$', '', name)
        if year:
        #    term = quote_plus(name) + '&year=' + year
            name = re.sub(year, ' ', name) #Wenn das Jahr im Namen auftaucht dann das Jahr löschen
        #else:

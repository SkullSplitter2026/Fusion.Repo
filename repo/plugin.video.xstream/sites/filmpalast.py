...
def load(): # Menu structure of the site plugin
    ...
    # Filme + Serien als Submenü (nur bei Alle/Deutsch)
...
def showMoviesMenu(): # Submenü Filme: Alle / Blockbuster / IMDB / Genre / A-Z
    ...
    cGui().addFolder(cGuiElement(cConfig().getLocalizedString(30517), SITE_IDENTIFIER, 'showValue'), params)      # A-Z
    cGui().setEndOfDirectory()
...
def _fetchAllSearchPages(startUrl, sGui=False):
    ...
    for page in range(maxPages):
        ...
        oRequest = cRequestHandler(currentUrl, ignoreErrors=(sGui is not False))
        sHtmlContent = oRequest.request()
        if not sHtmlContent:
            break
